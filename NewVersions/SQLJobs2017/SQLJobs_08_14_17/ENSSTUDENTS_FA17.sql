...
/****** Object:  Job [ENSSTUDENTS_FA17]    Script Date: 8/14/2017 10:33:46 AM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 8/14/2017 10:33:46 AM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'ENSSTUDENTS_FA17', 
		@enabled=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Processing]    Script Date: 8/14/2017 10:33:46 AM ******/
...
set @qrybk3 ='' FROM  CAMS_ENTERPRISE.dbo.CAMS_RptSROfferType1000_View AS CAMS_RptSROfferType1000_View WHERE (TextTerm = ''''FA-17'''')''
set @qrybkstore=@qrybk1+@qrybk2+@qrybk3

-- print  @qrybkstore


set @cmdstr= ''SQLCMD -S trocaire-sql01 -E  -s, -W -Q "SET ANSI_WARNINGS OFF; set nocount on; exec CAMS_enterprise.dbo.SendStudentEmailForENS"  > D:\CAMSEnterprise\ENS\StudentENSFile''
--print  @cmdstr
set @tmstp=LEFT(CONVERT(VARCHAR, GETDATE(), 120), 10)
set @globalcmdstr = @cmdstr+''_''+@tmstp+''.csv''
set @filename1=''D:\CAMSEnterprise\ENS\StudentENSFILE_''+@tmstp+''.csv''
print @globalcmdstr
...
set @topic=''Student List File for ''+@cterm+ '' as of ''+LEFT(CONVERT(VARCHAR, GETDATE(), 120), 10)
set @glfile=@filename1
set @tbody= ''Dear Nick, ''+CHAR(13)+CHAR(13)+CHAR(10)+''Please, find Attached combined Student and Faculty List File for ''+@cterm+ '' as of ''+LEFT(CONVERT(VARCHAR, GETDATE(), 120), 10)+
...
@recipients = ''auresn@trocaire.edu;etiennes@trocaire.edu'', 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'ENSSCHED', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=2, 
		@freq_subday_type=1, 
		...
		@schedule_uid=N'cd1cbd21-5e10-4741-8fa4-589cddc43190'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
