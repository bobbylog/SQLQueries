...
/****** Object:  Job [BN_TO_CAMS_JOB_SU-17]    Script Date: 8/14/2017 10:31:42 AM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 8/14/2017 10:31:42 AM ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Processing]    Script Date: 8/14/2017 10:31:42 AM ******/
					...
					begin
							...
							where BatchName=(''BN-ONLINE-''+cast(MONTH(GETDATE()) as varchar)+''-''+cast(DAY(GETDATE()) as varchar)+''-''+cast(YEAR(GETDATE()) as varchar))
								...
								DECLARE @name8 datetime
											...
											set @Tbody=''Batch ''+''BN-ONLINE-''+cast(MONTH(GETDATE()) as varchar)+''-''+cast(DAY(GETDATE()) as varchar)+''-''+cast(YEAR(GETDATE()) as varchar)+'' has been Uploaded with ''+cast(@rc as varchar)+'' transactions from Barnes and Nobles. Please, check out Billing batch in CAMS.''
											-- Send an email
						...
						begin
											...
											set @Topic=''No Transaction file was received today as of ''+cast(MONTH(GETDATE()) as varchar)+''-''+cast(DAY(GETDATE()) as varchar)+''-''+cast(YEAR(GETDATE()) as varchar)
											set @tbody= ''Dear Barnes and Nobles, ''+CHAR(13)+CHAR(13)+CHAR(10)+''We have not received a transaction file today. Please, contact Senghor Etienne at etiennes@trocaire.edu if this is the result of a transmission error. Disregards this message otherwise. '' +
														 CHAR(13)+CHAR(13)+CHAR(10)+''Thank you, ''+CHAR(13)+CHAR(13)+CHAR(10)+''Senghor E ''
						-- Send an email
											EXEC msdb.dbo.sp_send_dbmail 
											@profile_name=''TROCMAIL'',
											@recipients = ''jcipperly@bncollege.com;nrochez@bncollege.com;desbordesd@trocaire.edu;etiennes@trocaire.edu'', 
											-- @query = ''SELECT * FROM CAMS_ENTERPRISE.dbo.tmpStudentStatHistory WHERE DAY(SDATE)=DAY(GETDATE()) AND MONTH(SDATE)=MONTH(GETDATE()) AND YEAR(SDATE)=YEAR(GETDATE())'',
											@subject = @Topic ,
											@body = @tbody ;  
