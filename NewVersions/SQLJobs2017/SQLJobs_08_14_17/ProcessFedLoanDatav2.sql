...
/****** Object:  Job [ProcessFedLoanData]    Script Date: 2/19/2018 5:59:56 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 2/19/2018 5:59:56 PM ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Processing]    Script Date: 2/19/2018 5:59:56 PM ******/
...
set @RefFolder=''D:\FedLoans\Dec17Jan18\COD loan reports\''
...
DECLARE @gblcmd3 varchar(200)
DECLARE @gblcmd4 varchar(200)
DECLARE @gblcmd5 varchar(200)
...
DECLARE db_cursor CURSOR FOR  
    		...
    		set @gblcmd1=''copy /Y "'' + @RefFolder+''ProcessedLoanFilesTpl.csv" "''+@RefFolder+''ProcessedLoanFiles.csv"''
    		
    		set @gblcmd2=''type "''+@RefFolder+@name1+''" >> "''+@RefFolder+''ProcessedLoanFiles.csv"''

			set @gblcmd3=''findstr /v /i "Direct" "''+@RefFolder+''\ProcessedLoanFiles.csv" > "''+@RefFolder+''p1.csv"''

			set @gblcmd4=''copy /Y "''+@RefFolder+''\p1.csv" "''+@RefFolder+''\ProcessedLoanFiles.csv"''
    		
			set @gblcmd5=''del "''+@RefFolder+''\p1.csv"''

			--print @gblcmd
			
			EXEC  master..xp_cmdshell @gblcmd1
		    EXEC  master..xp_cmdshell @gblcmd2
			EXEC  master..xp_cmdshell @gblcmd3
			EXEC  master..xp_cmdshell @gblcmd4
			EXEC  master..xp_cmdshell @gblcmd5
			...
			begin
				...
				* 
				...
				from openrowset(''MSDASQL''
				 ...
				 DefaultDir=D:\FedLoans\Dec17Jan18\COD loan reports\''
	...
	delete from #FedLoanT where LOANID=''Direct Loan ID''
