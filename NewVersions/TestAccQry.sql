...
--DROP TABLE #Scorebase
    ...
    DECLARE @UStudentID varchar(12)
    DECLARe @UProgram varchar(60)
	...
	set @UStudentID='A0000025684'    
	set @UProgram='Nursing'    
...
SELECT SI.StudentUID, SI.StudentID, SI.FName, SI.Address1 As Address, 
     (SI.City+', ' +SI.State+' '+SI.ZipCode) as CSZ, SI.Phone1, 
      SI.IProg, SI.CProg, SI.TestName as TestName, 
       ...
       --case
       --   when NOT (SI.Score1 IS NULL OR SI.Score1 = 0) then CAST(CAST(SI.Score1 AS Decimal(5,1)) AS Char(10))
       --   else ''
       --end AS Head1,
       -- case 
		-- when si.ScoreName='Reading' then
		--	cast([dbo].[getStudentHSScoreByDateAndTestName] (SI.StudentID, SI.ScoreName, SI.CAMS_TestRefID, SI.TestDate) as CHAR(10))
		--	else ''
       --end 
      (select [dbo].[getStudentHSScoreByDateAndTestName] (SI.StudentID, 'Reading', SI.CAMS_TestRefID, SI.TestDate))
       AS Head1,
       --case
       --   when NOT (SI.Score2 IS NULL OR SI.Score2 = 0) then CAST(CAST(SI.Score2 AS Decimal(5,1)) AS Char(10))
       --   else ''
       --end AS Head2,              
       -- case 
		-- when si.ScoreName='Sentence Skills' then
			--cast([dbo].[getStudentHSScoreByDateAndTestName] (SI.StudentID, SI.ScoreName, SI.CAMS_TestRefID, SI.TestDate) as CHAR(10))
			(select [dbo].[getStudentHSScoreByDateAndTestName] (SI.StudentID, 'Sentence Skills', SI.CAMS_TestRefID, SI.TestDate))
	--		else ''
      -- end 
       AS Head2,  
         
       case
          when NOT (SI.Score3 IS NULL OR SI.Score3 = 0) then CAST(CAST(SI.Score3 AS Decimal(5,1)) AS Char(10))
          else ''
       end AS Head3,       
       --case
       --   when NOT (SI.Score4 IS NULL OR SI.Score4 = 0) then CAST(CAST(SI.Score4 AS Decimal(5,1)) AS Char(10))
       --   else ''
       -- end AS Head4,    
       --case 
        --when si.ScoreName='Algebra' then
        -- cast([dbo].[getStudentHSScoreByDateAndTestName] (SI.StudentID, SI.ScoreName, SI.CAMS_TestRefID, SI.TestDate) as CHAR(10))
       (select [dbo].[getStudentHSScoreByDateAndTestName] (SI.StudentID, 'Algebra', SI.CAMS_TestRefID, SI.TestDate))
       ...
       AS Head4,    
          ...
          --(select [dbo].[getStudentHSScoreByDateAndTestName] (SI.StudentID, SI.ScoreName, SI.CAMS_TestRefID, SI.TestDate)
          --  from #ScoreBase where StudentID=SI.StudentID AND ScoreName=SI.ScoreName and CAMS_TestRefID=SI.CAMS_TestRefID and
          --  TestDate=Si.TestDate) As Score23,
       case
