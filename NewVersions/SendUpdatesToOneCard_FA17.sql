...
/****** Object:  Job [SendUpdatesToOneCard_FA17]    Script Date: 8/14/2017 5:44:33 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 8/14/2017 5:44:33 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'SendUpdatesToOneCard_FA17', 
		@enabled=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Processing]    Script Date: 8/14/2017 5:44:33 PM ******/
...
AND ExpectedTermSeq Between ''B17L'' AND ''B17Q'' 
...
DECLARE @filename1 nvarchar(300)
	...
	set @cmdstr= ''SQLCMD -h-1 -S trocaire-sql01 -E  -s, -W -Q "set nocount on; EXEC CAMS_Enterprise.dbo.CTM_GenerateOneCardUpdateSet"  > D:\CAMSEnterprise\TMPGMExport\GMData''
	...
	EXEC  master..xp_cmdshell ''del /F /Q D:\CAMSEnterprise\TMPGMExport\*.*''
	EXEC  master..xp_cmdshell @globalcmdstr
	EXEC  master..xp_cmdshell ''D:''
	--EXEC  master..xp_cmdshell ''move D:\CAMSEnterprise\TMPGMExport\GMData*.cnm "D:\OneCard\CAMS Export"''
...
set @topic=''OneCard Sys has been updated today as of ''+LEFT(CONVERT(VARCHAR, GETDATE(), 120), 10)
set @glfile=@filename1
set @tbody= ''Dear HelpDesk, ''+CHAR(13)+CHAR(13)+CHAR(10)+''The OneCard System has been updated today ''+'' as of ''+LEFT(CONVERT(VARCHAR, GETDATE(), 120), 10)+''. Please, drop the attached file in the Onecard\CAMS Export Folder.''+
...
@recipients = ''daroisb@trocaire.edu;ziadalm@advance2000.com;etiennes@trocaire.edu'', 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'ONECARDSCHED', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=42, 
		@freq_subday_type=1, 
		...
		@schedule_uid=N'ad14a819-2fa4-4f2b-850f-abff8e215f8a'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
